...
export default function WordbookDetailClient({
  ...
  return (
    <section className='rounded-[16px] bg-white p-4 shadow-[rgba(19,19,22,0.7)_0px_1px_5px_-4px,rgba(34,42,53,0.08)_0px_0px_0px_1px,rgba(34,42,53,0.05)_0px_4px_8px_0px] md:p-5'>
      <div className='mb-4 flex flex-wrap items-center gap-2 border-b border-slate-200 pb-4'>
        ...
        <span className='ml-auto text-sm text-[#898989]'>
      ...
      {parentWordbook ? (
        <div className='mb-4 rounded-xl bg-white px-3 py-2 shadow-[rgba(19,19,22,0.7)_0px_1px_5px_-4px,rgba(34,42,53,0.08)_0px_0px_0px_1px,rgba(34,42,53,0.05)_0px_4px_8px_0px]'>
          <p className='text-xs text-[#898989]'>所属书籍</p>
          <Link
            href={`/wordbooks/${parentWordbook.id}`}
            className='mt-1 inline-flex text-sm font-semibold text-[#242424] hover:opacity-70'>
      ...
      {chapterItems.length > 0 ? (
        <div className='mb-4 rounded-xl bg-white p-3 shadow-[rgba(19,19,22,0.7)_0px_1px_5px_-4px,rgba(34,42,53,0.08)_0px_0px_0px_1px,rgba(34,42,53,0.05)_0px_4px_8px_0px]'>
          <p className='text-sm font-semibold text-[#242424]'>目录</p>
          <div className='mt-2 divide-y divide-slate-100 border-y border-slate-100'>
            {chapterItems.map(item => (
              <Link
                key={`chapter-${item.id}`}
                href={`/wordbooks/${item.id}`}
                className='flex items-center justify-between px-1 py-2 text-sm hover:opacity-70'>
                <span className='font-medium text-[#242424]'>{item.title}</span>
                <span className='text-xs text-[#898989]'>{item.count} 条</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className='mb-4 rounded-xl bg-white p-3 shadow-[rgba(19,19,22,0.7)_0px_1px_5px_-4px,rgba(34,42,53,0.08)_0px_0px_0px_1px,rgba(34,42,53,0.05)_0px_4px_8px_0px]'>
        <div className='flex flex-wrap items-center gap-2 border-b border-slate-100 pb-3'>
          <input
            ...
            className='h-10 min-w-[16rem] flex-1 border border-slate-200 px-3 text-sm text-[#242424] outline-none focus:border-slate-400'
          />
      ...
      {viewMode === 'list' ? (
        <div className='divide-y divide-slate-200 border-y border-slate-200 bg-white'>
          {filteredItems.map(item => (
            <div key={item.id} className='flex items-center gap-3 px-2 py-3 md:px-3'>
              <div className='flex-1 min-w-0'>
                <p className='truncate text-[28px] font-semibold tracking-tight text-[#242424] md:text-[32px]'>
                  {item.word}
                </p>
                <p className='mt-1 truncate text-xs text-[#898989]'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <Link
                  ...
                  className='inline-flex h-8 items-center border border-slate-200 px-2.5 text-xs font-semibold text-[#242424] hover:opacity-70'>
                  编辑
                </Link>
                <button
                  type='button'
                  ...
                  className='inline-flex h-8 items-center border border-slate-300 px-2.5 text-xs font-semibold text-[#242424] hover:opacity-70'>
                  移出
                </button>
              </div>
            </div>
