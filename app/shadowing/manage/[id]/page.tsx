...
import LessonQuestionsPanel from '@/app/collections/lesson/[lessonId]/LessonQuestionsPanel'
import LessonSiblingNav from '@/app/collections/lesson/[lessonId]/LessonSiblingNav'
import { getSpeakingEditData } from '@/lib/repositories/collection/manage'
import ShadowingTitleForm from '@/app/shadowing/manage/ShadowingTitleForm'
...
export default async function ManageShadowingEditPage({
  ...
  const lesson = await getSpeakingEditData(id)
    ...
    <main className='min-h-screen bg-gray-50 px-3 py-4 md:px-8 md:py-8'>
      <div className='mx-auto max-w-6xl'>
        <div className='mb-4 flex items-center gap-2 text-xs font-semibold md:mb-6 md:text-sm'>
          <Link href='/shadowing/manage' className='text-indigo-600 hover:text-indigo-700'>
            返回听力管理
          </Link>
        ...
        <LessonSiblingNav
          lessons={lesson.siblings}
          currentLessonId={lesson.id}
          hrefBase='/shadowing/manage'
        />
        ...
        <section className='mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:mb-6 md:p-6'>
          ...
          <ShadowingTitleForm id={lesson.id} title={lesson.title} />
        </section>
