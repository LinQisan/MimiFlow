...
import { listWordbooksTree } from '@/app/actions/content'
...
export default function ManageAnkiImportPage() {
  ...
  const [wordbookOptions, setWordbookOptions] = useState<
    Array<{ id: string; title: string; parentId: string | null; pathLabel: string }>
  >([])
  ...
  useEffect(() => {
    void (async () => {
      const rows = (await listWordbooksTree()) as WordbookTreeNode[]
      const map = new Map<string, WordbookTreeNode>(
      ...
      const buildPath = (id: string) => {
        ...
        while (cursor && map.has(cursor)) {
          const node: WordbookTreeNode = map.get(cursor)!
          ...
          <div className='mt-3 grid grid-cols-1 gap-3 md:grid-cols-2'>
            ...
            <label className='flex flex-col gap-2 text-sm font-semibold text-gray-700'>
              ...
              <select
                ...
                className='h-10 border border-gray-200 bg-white px-3 text-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100'>
                <option value=''>不选择（使用下方路径自动创建）</option>
                {wordbookOptions.map(option => (
        ...
        {summary && (
          <section className='mt-4 border-b border-emerald-200 bg-emerald-50 px-4 py-4'>
            ...
            <div className='mt-2 text-xs text-emerald-700'>
              ...
              {summary.notebookName || selectedWordbookPathLabel || notebookName || '未设置'} ｜ 分组：
