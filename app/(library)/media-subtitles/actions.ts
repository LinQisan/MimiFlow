...
async function resolveMediaMaterialByLegacyId(legacyId: string) {
  return prisma.material.findFirst({
    where: {
      type: MaterialType.MEDIA_SUBTITLE,
      id: { endsWith: `:${legacyId}` },
    },
    select: {
      id: true,
      title: true,
...
export async function updateMediaSubtitleDialogues(formData: FormData) {
  try {
    const legacyId = String(formData.get('legacyId') || '').trim()
    const rawDialogues = String(formData.get('dialoguesJson') || '').trim()
    const nextTitle = String(formData.get('title') || '').trim()

    if (!legacyId) return { success: false, message: '字幕 ID 缺失。' }
    if (!rawDialogues) return { success: false, message: '字幕内容不能为空。' }

    let parsed: unknown
    try {
      parsed = JSON.parse(rawDialogues)
    } catch {
      return { success: false, message: '字幕数据格式错误。' }
    }

    const normalizedRows = normalizeDialogueRows(parsed)
    if (normalizedRows.length === 0) {
      return { success: false, message: '请至少保留一行字幕文本。' }
    }

    const material = await resolveMediaMaterialByLegacyId(legacyId)
    ...
    const nextMaterialTitle = nextTitle || material.title
    ...
    revalidatePath('/media-subtitles')
    revalidatePath(`/media-subtitles/${legacyId}`)

    return { success: true, message: `已保存 ${normalizedRows.length} 行字幕。` }
  } catch (error) {
    console.error('updateMediaSubtitleDialogues failed:', error)
...
export async function deleteMediaSubtitleLine(legacyId: string, lineId: number) {
  try {
    if (!legacyId) return { success: false, message: '字幕 ID 缺失。' }
    if (!Number.isFinite(lineId) || lineId <= 0) {
      return { success: false, message: '字幕行 ID 无效。' }
