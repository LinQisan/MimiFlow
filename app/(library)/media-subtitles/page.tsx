      ...
      <div className='border-b border-slate-200 bg-white'>
        <div className='mx-auto flex max-w-7xl flex-col gap-5 px-4 py-6 md:px-6 lg:px-8'>
          <div className='flex flex-col gap-4 md:flex-row md:items-start md:justify-between'>
            <div className='max-w-3xl'>
              ...
              <p className='mt-3 text-sm leading-6 text-slate-600'>
                按作品管理电影与电视剧字幕。电视剧按季集排列，电影保留轻量列表，适合快速回到某一集、整理台词和继续阅读。
              </p>
            </div>
          ...
          <div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-4'>
            {[
              ['电视剧作品', tvGroups.length],
              ['电视剧集数', tvEpisodeCount],
              ['电影作品', movieGroups.length],
              ['字幕总行数', totalDialogues],
              ...
              <section className='border border-slate-200 bg-white'>
                ...
                {tvGroups.length === 0 ? (
                  ...
                  <div className='divide-y divide-slate-200'>
                    {tvGroups.map(group => (
                      <details key={`tv-group-${group.key}`} className='group' open>
                                ...
                                <div className='grid gap-2 md:grid-cols-2 xl:grid-cols-3'>
                                  {season.items.map(item => (
                                    <Link
                                      key={item.id}
                                      href={item.href}
                                      className='block rounded-lg border border-slate-200 bg-white p-3 transition hover:border-teal-300 hover:bg-teal-50/40'>
            ...
            <aside className='space-y-5'>
              <section className='border border-slate-200 bg-white'>
                <div className='border-b border-slate-200 px-4 py-4'>
                  ...
                  <p className='mt-1 text-sm text-slate-500'>快速回到刚整理的字幕。</p>
                </div>
              ...
              <section className='border border-slate-200 bg-white p-4'>
                <h2 className='text-base font-black text-slate-950'>整理建议</h2>
                <div className='mt-3 space-y-3 text-sm leading-6 text-slate-600'>
                  <p>
                    电视剧的作品名保持一致，页面就会自动归并为同一部剧。
                  </p>
                  <p>
                    季、集填写数字，列表会按自然顺序显示。
                  </p>
                  <p>
                    作品名写法不一致时，可以在详情页逐条修正。
                  </p>
                </div>
              </section>
            </aside>
