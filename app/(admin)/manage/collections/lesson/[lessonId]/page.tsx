    ...
    <main className='min-h-screen bg-gray-50 px-3 py-4 md:px-8 md:py-8'>
      <div className='mx-auto max-w-6xl'>
        <div className='mb-4 flex items-center gap-2 text-xs font-semibold md:mb-6 md:text-sm'>
          <Link
            href={`/manage/collections/${lesson.collectionId}`}
        ...
        <section className='mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:mb-6 md:p-6'>
          ...
          <p className='mt-1 text-xs text-gray-500 md:text-sm'>
            归属：
            {lesson.subtitleMeta.sourceType === 'TV' ? '电视剧' : '电影'}
            {lesson.subtitleMeta.workTitle ? ` · ${lesson.subtitleMeta.workTitle}` : ''}
            {lesson.subtitleMeta.sourceType === 'TV' && lesson.subtitleMeta.season
              ? ` · 第${lesson.subtitleMeta.season}季`
              : ''}
            {lesson.subtitleMeta.sourceType === 'TV' && lesson.subtitleMeta.episode
              ? ` · 第${lesson.subtitleMeta.episode}集`
              : ''}
          </p>
          <p className='mt-2 inline-block rounded-md bg-gray-100 px-2 py-1 font-mono text-[11px] text-gray-500 break-all md:text-xs'>
            音频：
            {lesson.subtitleMeta.noAudio
              ? '无音频（仅字幕）'
              : lesson.audioFile || '未设置'}
          </p>
        </section>
