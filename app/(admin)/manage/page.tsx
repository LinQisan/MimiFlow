...
export default async function ManageHomePage() {
  ...
  const todos = [
    { label: '听力缺少题目', count: missingQuestions, href: '/manage/listening', tone: 'rose' },
    { label: '听力未设置所属問題', count: missingSection, href: '/manage/listening', tone: 'amber' },
    { label: '疑似重复听力', count: duplicateGroups, href: '/manage/listening', tone: 'amber' },
    { label: '跟读未归类', count: unclassifiedShadowing, href: '/manage/shadowing', tone: 'violet' },
    { label: '阅读文章缺少题目', count: readingMissingQuestions, href: '/manage/reading?status=missingQuestions', tone: 'rose' },
  ] as const
      ...
      <div className='mx-auto max-w-7xl space-y-5'>
        <header className='border-b border-slate-200 pb-6'>
          <div className='flex flex-wrap items-end justify-between gap-3'>
            ...
            <div className='min-w-28 border-l border-slate-300 pl-5 text-right'>
              ...
              <p className='mt-1 text-[11px] uppercase tracking-[0.12em] text-slate-500'>{totalTodo ? '项待处理' : '当前无异常'}</p>
            </div>
          </div>
        </header>
        ...
        <section>
          <div className='mb-3 flex items-center justify-between'>
            ...
            <Link href='/manage/system' className='text-sm font-semibold text-slate-500 hover:text-slate-900'>系统检查 →</Link>
          </div>
          <div className='grid border-y border-slate-200 sm:grid-cols-2 lg:grid-cols-3 lg:divide-x lg:divide-slate-200'>
            {todos.map(item => (
              <Link key={item.label} href={item.href} className='group border-b border-slate-200 px-2 py-6 transition hover:bg-slate-50 lg:px-6'>
                <div className='flex items-start justify-between gap-3'>
                  <div>
                    <p className='text-sm font-bold text-slate-700'>{item.label}</p>
                    <p className='mt-2 text-xs text-slate-400'>{item.count ? '点击进入处理' : '无需处理'}</p>
                  </div>
                  <span className={`text-3xl font-black ${item.count ? 'text-rose-600' : 'text-emerald-600'}`}>{item.count}</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        <section className='grid border-y border-slate-200 md:grid-cols-3 md:divide-x md:divide-slate-200'>
          {[
            ...
            <Link key={String(href)} href={String(href)} className='border-b border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 last:border-b-0 md:border-b-0'>
              <span>{label}</span><span className='float-right text-slate-400'>{count}</span>
            </Link>
          ))}
        </section>
      </div>
