...
export default function EditArticleUI({ article }: { article: any }) {
  ...
  const [questions, setQuestions] = useState<any[]>(article.questions || [])
  ...
  // 🌟 修复 1：将弹窗对象改为存储正在编辑的题目 ID（实现内联编辑）
  const [editingQuestionId, setEditingQuestionId] = useState<string | null>(
    null,
  )

  // 🌟 修复 2：恢复控制新增按钮下拉菜单的状态
  ...
  const handleAddNewQuestion = (
    ...
    setEditingQuestionId(newQ.id) // 新增后立刻进入内联编辑模式
  ...
  const handleCreateBlankQuestion = () => {
    ...
    setSelection(null)
    setEditingQuestionId(newQuestion.id) // 划词后立刻进入内联编辑模式
  }
  ...
  const handleSaveArticle = async () => {
    setIsSaving(true)
    setTimeout(() => {
      void dialog.alert('文章、题目及排序已保存成功！(UI演示)')
      setIsSaving(false)
    }, 800)
  }

  return (
    <div className='p-4 md:p-8 max-w-400 mx-auto pb-32 animate-in fade-in duration-500 relative'>
      {selection && !isMobile && (
        <div
          className='fixed z-100 bg-gray-900 text-white px-4 py-2.5 rounded-2xl shadow-2xl shadow-indigo-500/10 flex items-center gap-3 animate-in zoom-in-95 duration-200'
      ...
      <div className='mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 bg-gray-50/90 backdrop-blur-sm z-40 py-4 -mx-4 px-4'>
        <div>
          <Link
            href={`/admin/level/${article.category?.levelId}`}
            className='text-xs px-3 py-1 bg-gray-100 text-gray-500 font-bold rounded-lg hover:bg-gray-200 transition-colors mb-2 inline-block'>
            &larr; 返回列表
          </Link>
        ...
        <button
          ...
          disabled={isSaving}
          className='px-8 py-3 bg-gray-900 text-white font-black rounded-xl hover:bg-gray-800 transition-all shadow-sm active:scale-95 disabled:opacity-50 flex items-center gap-2'>
          {isSaving ? '正在保存中...' : '💾 保存所有修改'}
        </button>
      </div>
      ...
      <div className='flex flex-col lg:flex-row gap-6 md:gap-8 items-start relative z-10'>
        ...
        <div className='w-full lg:flex-3 flex flex-col gap-6 relative'>
          ...
          <p className='text-xs font-bold text-indigo-400 absolute -bottom-5.5 left-3 z-20'>
            💡 技巧：在上方文本框中用鼠标划选词汇，即可一键生成填空题。
          </p>
        </div>

        {/* 右侧：题目管理区 */}
        <div className='w-full lg:flex-2 bg-gray-50 rounded-3xl border border-gray-200 p-5 md:p-6 lg:sticky lg:top-[12vh] max-h-[85vh] flex flex-col'>
                            ...
                            <div className='flex gap-2.5 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity'>
                              ...
                              <ActionInterceptor>
                                <button
                                  ...
                                  className='text-xs text-red-400 hover:text-red-600 font-bold bg-red-50 px-2 py-1 rounded-md'>
                                  🗑️
                                </button>
                              </ActionInterceptor>
                            </div>
