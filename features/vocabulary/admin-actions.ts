...
export async function searchGlobalCorpus(
  ...
  const results = [
    ...listeningMaterials.flatMap(item => {
      const payload = decodeMaterialPayloadRecord(item.type, item.contentPayload)
      const dialogues = Array.isArray(payload.dialogues)
        ? (payload.dialogues as Record<string, unknown>[])
        : []
      const matched = dialogues.find(dialogue => readString(dialogue.text).includes(k))
      const fallbackText = readString(matched?.text) || getMaterialDisplayTitle(
        item.type,
        item.title,
        item.contentPayload,
        item.id,
