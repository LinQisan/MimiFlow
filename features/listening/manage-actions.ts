...
const speakingMetaSchema = z.object({
  id: materialIdSchema,
  title: z.string().trim().min(1, '标题不能为空。'),
  chapterName: z.string().trim().catch(''),
  audioFile: z.string().trim().min(1, '音频路径不能为空。'),
})
...
export async function updateSpeakingMeta(formData: FormData) {
  try {
    const { id: maybeId, title, chapterName, audioFile } = parseInput(
      speakingMetaSchema,
    ...
    await prisma.material.update({
      ...
      data: {
        ...
        contentPayload: encodeMaterialPayload(material.type, nextPayload),
      },
    })
    ...
    return actionSuccess({}, '标题与章节名已更新。')
  } catch (error) {
