...
export async function updateShadowingMaterial(formData: FormData) {
  try {
    ...
    if (!materialIdInput) return { success: false, message: 'id 缺失。' }
    ...
    const materialId = materialIdInput
    ...
    const upload = readJsonRecord(metadata.upload)
    ...
    await prisma.$transaction(async tx => {
      await tx.material.update({
        ...
        data: {
          title,
          type: materialType,
          contentPayload: encodeMaterialPayload(materialType, nextPayload),
