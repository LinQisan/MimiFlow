...
export type NewsSeries = '' | '天声人語' | '社説' | '春秋'
...
export const getNewsSeriesSource = (series: NewsSeries): NewsSource => {
  if (series === '天声人語') return '朝日'
  if (series === '社説' || series === '春秋') return '日経'
  return ''
}

export const toLegacyNewsSeries = (type: NewsType, column: NewsColumn): NewsSeries => {
  if (type === 'editorial') return '社説'
...
export const normalizeNewsMetadata = (input: {
  ...
  const legacySeries = (['天声人語', '社説', '春秋'].includes(input.newsSeries || '')
    ? input.newsSeries
    : '') as NewsSeries
  const legacySource = getNewsSeriesSource(legacySeries)
  ...
  const source = (['日経', '朝日'].includes(input.newsSource || '')
    ? input.newsSource
    : legacySource || collectionSource) as NewsSource
  const legacyType: NewsType = legacySeries === '社説'
    ? 'editorial'
    : legacySeries === '春秋' || legacySeries === '天声人語'
      ? 'column'
      : ''
  const type = (['news', 'editorial', 'column'].includes(input.newsType || '')
    ? input.newsType
    : legacyType) as NewsType
  const legacyColumn = legacyType === 'column' ? legacySeries : ''
  const column = (['春秋', '天声人語'].includes(input.newsColumn || '')
    ? input.newsColumn
    : legacyColumn) as NewsColumn

  return {
    source,
    type,
    section: (input.newsSection || input.pageNumber || '').trim(),
    column,
