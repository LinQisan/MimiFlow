...
export async function searchGlobalContent(
            ...
            OR: [
              ...
              { partsOfSpeech: { contains: primaryToken } },
              {
                senses: {
                  some: {
                    definitions: {
                      some: { definition: { contains: primaryToken } },
                    },
                  },
                },
              },
  ...
  const rankedVocabRows = sortByScore(
    ...
    item => [
      ...
      item.pronunciations,
      item.partsOfSpeech,
      ...listVocabularyMeanings(item.senses),
  ...
  rankedVocabRows.forEach(item => {
    ...
    const meanings = listVocabularyMeanings(item.senses).slice(0, 2)
    const pronunciations = parseJsonStringList(item.pronunciations).slice(0, 1)

    const wordKey = normalizeVocabularyWord(item.word)
  ...
  rankedSentenceRows.forEach(sentence => {
    sentence.links.forEach(link => {
      ...
      const wordKey = normalizeVocabularyWord(vocabulary.word)
      ...
      const meanings = listVocabularyMeanings(vocabulary.senses).slice(0, 2)
