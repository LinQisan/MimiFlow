...
import { replaceVocabularyMeaningDefinitions } from '@/modules/knowledge/vocabulary/server/meaning-writer'
...
export async function getVocabulariesPagedAdmin(
        ...
        OR: [
          ...
          { partsOfSpeech: { contains: search } },
          {
            senses: {
              some: {
                definitions: {
                  some: { definition: { contains: search } },
                },
              },
            },
          },
  ...
  const items: VocabularyRecordForAdmin[] = rows.map(item => {
    ...
    return {
      ...
      meanings: listVocabularyMeanings(item.senses),
  ...
  try {
    ...
    const sentenceEdits = (payload.sentences || [])
      ...
      .map(sentence => ({
        ...
        posTags: normalizeVocabularySentencePosTags(sentence.posTags),
      }))
    ...
    const updated = await prisma.$transaction(async tx => {
      ...
      if (shouldUpdateMeta) {
        await replaceVocabularyMeaningDefinitions(
          tx,
          [{ vocabularyId: vocabId, meanings }],
          '管理页编辑',
        )
      }
  ...
  try {
    ...
    const csv = serializeVocabularyCsv(wordbook.entries.map(({ vocabulary, jlpt }) => {
      ...
      return {
        ...
        释义: listVocabularyMeanings(vocabulary.senses).join(' | '),
        ...sentenceColumns,
      }
    }))
    return {
  ...
  try {
    ...
    const updates = rows.map(row => {
      ...
      return {
        ...
        jlpt: normalizeVocabularyJlpt(row.JLPT),
    ...
    for (let offset = 0; offset < updates.length; offset += 200) {
      ...
      await prisma.$transaction(tx =>
        replaceVocabularyMeaningDefinitions(
          tx,
          chunk.map(row => ({
            vocabularyId: row.id,
...
export async function mergeVocabularyDuplicateGroupAdmin(
  ...
  try {
    ...
    await prisma.$transaction(async tx => {
      ...
      let nextSenseOrder =
        Math.max(-1, ...keep.senses.map(sense => sense.order)) + 1
      for (const source of sources) {
        await tx.vocabularyRelation.updateMany({
          where: { targetVocabularyId: source.id },
          data: { targetVocabularyId: keepId },
        })
        for (const sense of source.senses) {
          await tx.vocabularySense.update({
            where: { id: sense.id },
            data: { vocabularyId: keepId, order: nextSenseOrder },
          })
          await tx.vocabularyDefinition.updateMany({
            where: { senseId: sense.id },
            data: { vocabularyId: keepId },
          })
          await tx.vocabularyRelation.updateMany({
            where: { senseId: sense.id },
            data: { vocabularyId: keepId },
        ...
        for (const link of source.sentenceLinks) {
          ...
          const mergedTags = normalizeVocabularySentencePosTags([
