...
export default function ListeningSentenceRow({
  ...
  return (
    <div
      id={`sentence-${item.id}`}
      className='group grid scroll-mt-36 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 md:gap-3'>
      <div
        role='button'
        tabIndex={0}
        ...
        <div className='flex min-w-0 gap-3'>
          <span className='mt-0.5 w-5 shrink-0 text-right text-[11px] font-semibold tabular-nums text-slate-400' aria-hidden='true'>
            {sequence}
          </span>
          <div
      ...
      <div className='flex shrink-0 flex-col gap-1.5 sm:flex-row'>
        <button
          ...
          aria-label={canAddToReview ? `收藏第 ${sequence} 句` : '先完成释义匹配'}
